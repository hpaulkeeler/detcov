% function LPalmReduced=funPalmReducedL(L,indexPalm) 
...
% in the paper by Borodin and Rains[1], but a similar result appears 
...
%
% OUTPUTS:
% LPalmReduced= The (reduced) Palm version of the L matrix, which is a square 
...
identTemp=eye(sizeL); %identity matrix
identTemp(indexPalm,indexPalm)=0;%some ones set to zero
invLBoro=eye(sizeL)/(identTemp+L); 
identTemp=eye(length(indexRemain));

%Borodin and Rains construction -- see equation (24) in [1]
LPalmReduced=identTemp/(invLBoro(indexRemain,indexRemain))-identTemp;
end
